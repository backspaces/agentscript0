(function() {
  ...
  Color = {
    ...
    typedColor: (function() {
      ...
      typedColor = function(r, g, b, a, map, index) {
        var ua;
        if (a == null) {
          a = 255;
        }
  ...
  colorMixin = function(obj, colorName, colorDefault, colorMap, colorType) {
    var colorMapName, colorPropName, colorTitle, getterName, proto, ref, setterName;
    if (colorMap == null) {
      colorMap = null;
    }
    if (colorType == null) {
    ...
    if (proto[setterName] == null) {
      proto[setterName] = function(r, g, b, a) {
        ...
        if (g === void 0) {
          ...
          if (this.hasOwnProperty(colorPropName) && colorType === "typed") {
            color = this[colorPropName];
  ...
  shapes = Shapes = (function() {
    ...
    return {
      ...
      shapeToSprite: function(name, color, size, strokeColor) {
        var colorCss, ctx, foundSlot, img, index, shape, slot, slotSize, spriteSize, strokeCss, strokePadding, x, y;
        colorCss = Color.convertColor(color, "css");
        if (strokeColor != null) {
          strokeCss = Color.convertColor(strokeColor, "css");
        }
        spriteSize = Math.ceil(size);
        strokePadding = 4;
        slotSize = spriteSize + strokePadding;
        shape = this[name];
        index = shape.img != null ? name : name + "-" + colorCss;
    ...
    AgentSet.prototype.inCone = function(o, heading, cone, radius, meToo) {
      ...
      if (this.model.patches.isTorus) {
        ...
        return this.asSet((function() {
          var len, m, results;
          results = [];
          for (m = 0, len = rSet.length; m < len; m++) {
            a = rSet[m];
  ...
  Patches = (function(superClass) {
    ...
    Patches.prototype.setNeighbors = function() {
      var len, m, n, p, results;
      results = [];
      for (m = 0, len = this.length; m < len; m++) {
        p = this[m];
        p.n = this.patchRect(p, 1, 1);
        results.push(p.n4 = this.asSet((function() {
          var aa, len1, ref, results1;
          ref = p.n;
          results1 = [];
  ...
  Agent = (function() {
    ...
    Agent.prototype.inRadius = function(aset, radius, meToo) {
      if (meToo == null) {
        meToo = false;
      }
      return aset.inRadius(this.p, radius, meToo);
    };
  ...
  Animator = (function() {
    function Animator(model, rate1, multiStep) {
      ...
      this.multiStep = multiStep != null ? multiStep : this.model.world.isHeadless;
      this.animateDraws = bind(this.animateDraws, this);
      this.animateSteps = bind(this.animateSteps, this);
      this.isHeadless = this.model.world.isHeadless;
      this.reset();
    }
